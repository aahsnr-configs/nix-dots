...
** Language Server Protocol (basedpyright)
Configure lsp-mode to use basedpyright, which offers more features than the default pyright.
#+begin_src emacs-lisp
(use-package! lsp-pyright
  :init
  ;; Disable ruff-lsp BEFORE lsp-mode starts
  (setq lsp-disabled-clients '(ruff-lsp ruff))
  ;; Use basedpyright from nix-shell environment
...
** Code Formatting (Apheleia + Ruff)
Decouple code formatting from LSP and use apheleia with ruff for fast, reliable formatting.
#+begin_src emacs-lisp
;; Disable LSP formatting for Python
(setq-hook! 'python-mode-hook +format-with-lsp nil)
(setq-hook! 'python-ts-mode-hook +format-with-lsp nil)

;; Configure apheleia to use ruff for Python formatting
;; Ruff formatter is already built-in to apheleia, we just need to associate it
(after! apheleia
  (setf (alist-get 'python-mode apheleia-mode-alist) '(ruff))
  (setf (alist-get 'python-ts-mode apheleia-mode-alist) '(ruff)))
#+end_src

** Syntax Checking (Flycheck + Ruff + Mypy)
...
(after! flycheck
  (defun my/python-flycheck-setup ()
    "Set up flycheck for Python with ruff and mypy."
    ;; Disable LSP diagnostics to avoid conflicts
...
** Debugging (Dape for Python)
Dape comes with built-in debug adapter configurations for Python using debugpy. No additional configuration is required for basic debugging functionality. Simply install debugpy (`pip install debugpy`) and use `M-x dape` to start debugging, selecting one of the Python configurations (e.g., `debugpy`).

If you need custom debugging configurations (e.g., with specific arguments or environment variables), you can add them:
#+begin_src emacs-lisp
(after! dape
  ;; Example: Custom Python debugging configuration with arguments
  (add-to-list 'dape-configs
               `(debugpy-args
                 ...
                 :args ["arg1" "arg2"])))  ; Your custom arguments here
#+end_src

** Optimize Org Mode Jupyter Integration
Disable Jupyter's code completion features within Org mode to avoid conflicts with the new LSP setup.
#+begin_src emacs-lisp
(after! jupyter
  ;; Remove jupyter completion from completion-at-point-functions
  ;; when in org-src-mode for Python to avoid conflicts with LSP completion
  (add-hook 'org-src-mode-hook
    (lambda ()
      (when (and (derived-mode-p 'python-mode 'python-ts-mode)
                 (bound-and-true-p jupyter-current-client))
        ;; Remove jupyter-org-completion-at-point from the list
