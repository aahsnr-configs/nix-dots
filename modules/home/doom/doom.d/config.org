...
;;; Removed fontspec
(after! org
  ;; Clean up default packages list - remove fontspec/inputenc/fontenc
  ;; These will cause issues even with Tectonic if not handled properly
  ...
  ;; Add Tectonic to preview process list with corrected configuration
  (add-to-list 'org-preview-latex-process-alist
               '(tectonic
                 ...
                 ;; Compile with Tectonic - use %O for output directory
                 :latex-compiler
                 ("tectonic -X compile -f plain --outdir %o %f")
                 ;; Convert PDF to PNG - the PDF will be in %o with same basename as %f
                 :image-converter
                 ("convert -density %D -trim -antialias %o/%b.pdf -quality 100 %O")))

  ;; Set Tectonic as default
  ...
  ;; LaTeX header for preview - keep it minimal, no fontspec
